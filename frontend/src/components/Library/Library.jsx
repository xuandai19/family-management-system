      ...
      <div className="flex justify-between items-center mb-8 text-left">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Lưu trữ bảo vật và tư liệu quý của dòng họ
          </p>
        </div>
      ...
      <div className="bg-white/70 backdrop-blur-md p-5 rounded-[2rem] shadow-xl shadow-slate-100 border border-white mb-8 flex flex-col xl:flex-row gap-5 items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:flex bg-slate-100 p-1.5 rounded-2xl gap-1">
            <button className="px-4 py-2 text-xs font-bold text-slate-600 bg-white rounded-xl shadow-sm">
              Tất cả
            </button>
            <button className="px-4 py-2 text-xs font-bold text-slate-500 hover:text-teal-600 transition-colors">
              PDF
            </button>
            <button className="px-4 py-2 text-xs font-bold text-slate-500 hover:text-teal-600 transition-colors">
              Ảnh
      ...
      >
        {LIBRARY_ITEMS.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border-2 ${style.border} p-6 transition-all duration-500 group hover:shadow-2xl hover:-translate-y-3 relative overflow-hidden text-left`}
            >
              ...
              <div className="flex justify-between items-start mb-6">
                <div
                  className={`p-4 rounded-2xl shadow-lg transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6 ${style.bg} ${style.color}`}
                >
                  {style.icon}
                </div>
                <span
                  className={`text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-sm ${style.bg} ${style.color}`}
                >
                  {item.type}
                </span>
              </div>
              ...
              <div className="flex gap-3 relative z-10 pt-4 border-t border-slate-50">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-3 text-xs font-black uppercase tracking-widest text-slate-500 hover:bg-slate-50 rounded-xl transition-all"
                >
                  <Eye size={16} strokeWidth={3} /> Xem
                </button>
                <button
                  className={`flex-1 flex items-center justify-center gap-2 py-3 text-xs font-black uppercase tracking-widest text-white rounded-xl shadow-lg transition-all active:scale-90 ${style.btn}`}
                >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[110] p-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in duration-300 text-left">
            ...
            <div className="p-10 space-y-6">
              ...
              <div className="border-4 border-dashed border-slate-100 rounded-[2rem] p-10 flex flex-col items-center justify-center gap-4 hover:border-teal-200 hover:bg-teal-50/30 transition-all group cursor-pointer">
                ...
                <div className="text-center">
                  <p className="font-black text-slate-700 uppercase text-xs tracking-widest">
                    Nhấn để chọn file hoặc kéo thả
                  </p>
                  <p className="text-slate-400 text-[10px] mt-1 font-bold">
                    PDF, JPG, PNG, DOCX (Tối đa 25MB)
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 outline-none focus:ring-2 focus:ring-teal-500 font-bold text-slate-700 appearance-none cursor-pointer">
                    <option>Sử tích</option>
                    <option>Gia phả gốc</option>
                    <option>Hình ảnh cổ</option>
                    <option>Văn bản khác</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 ml-1 tracking-widest">
            ...
            <div className="p-8 bg-slate-50 border-t flex justify-end gap-4">
              <button
                onClick={() => setShowUploadModal(false)}
                className="px-6 py-3 font-black text-slate-400 hover:text-slate-600 transition-colors uppercase text-xs tracking-widest"
              >
                Hủy bỏ
              </button>
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`p-3 rounded-2xl text-white ${
                    getFileStyle(selectedPreview.type).btn
                  }`}
                >
                  <FileText size={24} />
                </div>
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">
                    Đang xem trước • {selectedPreview.size}
                  </p>
                </div>
              </div>
              <button
                onClick={() => setSelectedPreview(null)}
                className="p-3 bg-white hover:bg-red-50 hover:text-red-500 rounded-full shadow-md active:scale-90 transition-all"
              >
                <X size={24} />
              </button>
            </div>
            <div className="flex-1 bg-slate-200 overflow-y-auto p-10 flex items-center justify-center italic text-slate-400">
              Vùng xem trước tài liệu...
            </div>
            <div className="p-8 bg-white border-t flex justify-end gap-4">
              <button
                onClick={() => setSelectedPreview(null)}
                className="px-8 py-3 font-black text-slate-400 hover:text-slate-600 transition-colors uppercase text-xs tracking-widest"
              >
                Đóng
              </button>
