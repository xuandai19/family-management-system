    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-lg relative z-10 shadow-2xl border-2 border-emerald-100 overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="bg-[#009a66] p-6 text-white flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="hover:rotate-90 transition-all p-1 bg-white/20 rounded-full"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-8 space-y-5 text-left">
          ...
          {/* Vai trò / Chức vụ */}
          <div>
            <label className="block text-[10px] font-black uppercase text-slate-400 mb-2 ml-1 tracking-widest italic">
              Phân quyền vai trò
            </label>
            <div className="relative">
              <ShieldCheck
          ...
          <div className="pt-4 space-y-3">
            <button
              onClick={onSave}
              className="w-full bg-[#009a66] text-white py-4 rounded-2xl font-black uppercase tracking-[0.2em] shadow-lg hover:bg-emerald-700 hover:scale-[1.01] active:scale-95 transition-all flex items-center justify-center gap-3 border-b-8 border-emerald-900"
            >
            ...
            <p className="text-[9px] text-center text-slate-400 font-bold uppercase italic leading-tight">
              Lưu ý: Hệ thống sẽ gửi một email thông báo tài khoản cho thành
              viên mới này.
            </p>
          </div>
        </div>
  ...
  const [admins, setAdmins] = useState([
    {
      id: 1,
      name: "Thanh Thủy",
      email: "thuy@giapha.vn",
      role: "QTV",
      date: "05/09/2024 14:32:21",
    },
    {
      id: 2,
      name: "Văn Kiểm",
      email: "kiem@giapha.vn",
      role: "BTV",
      date: "24/10/2024 11:08:11",
    },
    {
      id: 3,
      name: "Nguyễn Thu Hà",
  ...
  const auditLogs = [
    {
      id: 1,
      user: "Thanh Thủy",
      action: "Cập nhật cây gia phả Họ Nguyễn",
      time: "10:15 - 26/12/2025",
    },
    {
      id: 2,
      user: "Văn Kiểm",
      action: "Lập phiếu chi: Tu bổ lăng mộ",
      time: "09:30 - 26/12/2025",
    },
  ];
    ...
    <div className="p-4 lg:p-8 bg-[#f8fafc] min-h-screen font-sans text-left relative overflow-x-hidden">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-[10px] text-slate-400 font-bold mt-2 ml-1 uppercase tracking-widest italic">
            An toàn dữ liệu & Minh bạch dòng tộc
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 w-full lg:w-auto">
          ...
          >
            <History size={16} />{" "}
            {showLogs ? "Đóng nhật ký" : "Lịch sử thay đổi"}
          </button>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div
          className={`${
            showLogs ? "lg:col-span-2" : "lg:col-span-3"
          } bg-white rounded-[2rem] shadow-xl border border-emerald-50 overflow-hidden transition-all duration-500`}
        >
          <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
            ...
            <div className="relative">
              <input
                ...
                className="pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-xs outline-none focus:ring-2 focus:ring-emerald-500 w-48 shadow-sm"
              />
              <Search
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="bg-[#009a66] text-white">
                  ...
                  <th className="px-6 py-4 text-[10px] font-black uppercase text-center">
                    Chức vụ
                  </th>
                  <th className="px-6 py-4 text-[10px] font-black uppercase text-center">
                    Ngày tạo
              ...
              <tbody className="divide-y divide-slate-50">
                {admins.map((admin) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-5 text-center text-slate-500 font-bold text-xs">
                      {admin.date}
                    </td>
                    <td className="px-6 py-5 text-center">
                      <div className="flex justify-center gap-2 lg:opacity-0 lg:group-hover:opacity-100 transition-all">
                        <button className="p-2 text-blue-500 bg-blue-50 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
                          <RefreshCcw size={14} />
                        </button>
                        <button
                          onClick={() => triggerToast("Đã xóa quản trị viên!")}
                          className="p-2 text-rose-500 bg-rose-50 rounded-lg hover:bg-rose-600 hover:text-white transition-all"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
        ...
        {showLogs && (
          <div className="bg-slate-900 rounded-[2rem] p-6 shadow-2xl text-white animate-in slide-in-from-right duration-500 border-t-8 border-[#009a66]">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="font-black uppercase italic tracking-widest text-xs">
                Nhật ký hoạt động
              </h3>
            </div>
            <div className="space-y-4">
              {auditLogs.map((log) => (
                <div
                  key={log.id}
                  className="p-4 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all cursor-default text-left"
                >
                  <p className="text-[11px] font-black text-emerald-400 uppercase tracking-tighter mb-1">
                    {log.user}
                  </p>
                  <p className="text-xs font-medium text-slate-300 mb-2 leading-relaxed">
                    {log.action}
                  </p>
